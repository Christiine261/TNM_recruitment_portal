									...
									<table  id="example1"  class="table table-hover table-striped table-bordered table-responsive">
										...
										<?php
											...
											if (isset($_POST['ssubmit']) == 'POST') {
												...
												$sq = "SELECT * FROM job_applications WHERE job_id = '$jobTitle' AND status = 'pending' ORDER BY score DESC";;
												$res = $conn->query($sq);
												...
												while (($ro = $res->fetch_assoc()) && ($counts < $shortlistCount)) {
													...
													$inss = "UPDATE job_applications SET status = 'shortlisted' WHERE job_id = '$job_id'";
													$nns = $conn->query($inss);
